        ...
        if (.not. EddyFlowProj%fix_out_format) then
            ...
            do gas = co2, gas4
                if (gas /= h2o) then
                    ...
                    if (gas == gas4) then
                        ...
                        if(fcc_var_present(gas)) call AddDatum(header3, '[' // char(194) // char(181) // 'mol+1s-1m-2]', separator)
                    end if
                else
                    if(fcc_var_present(gas)) call AddDatum(header1, '', separator)
                    if(fcc_var_present(gas)) call AddDatum(header2, e2sg(gas)(1:len_trim(e2sg(gas))) // 'strg', separator)
            ...
            do gas = co2, gas4
                ...
                if (gas == gas4) then
                    ...
                    if(fcc_var_present(gas)) call AddDatum(header3, '[mmol+1m-3],[' // char(194) // char(181) // &
