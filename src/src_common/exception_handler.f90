...
subroutine ExceptionHandler(error_code)
    ...
    select case (error_code)
        ...
        case(113)
            call LogSayList(' Warning(113)> The conditional eddy covariance pairing named on the')
            call LogSayList(' Warning(113)> line above takes its water from a different analyser')
            call LogSayList(' Warning(113)> than its carbon dioxide, because that analyser carries')
            call LogSayList(' Warning(113)> no hygrometer.')
            call LogSayList(' Warning(113)> The method sorts air parcels into octants by the signs')
            call LogSayList(' Warning(113)> of the vertical wind, the water and the carbon dioxide')
            call LogSayList(' Warning(113)> together, so those octants are now built from two')
            call LogSayList(' Warning(113)> instruments - which reach the sensor at different time')
            call LogSayList(' Warning(113)> lags and through different spectral responses. A parcel')
            call LogSayList(' Warning(113)> can be sorted into the wrong octant on that alone.')
            call LogSayList(' Warning(113)> The partition still runs, and is still worth reading,')
            call LogSayList(' Warning(113)> but it is a weaker measurement than a pairing whose two')
            call LogSayList(' Warning(113)> channels sit on one instrument.')

        case(114)
            call LogSayList(' Warning(114)> Conditional eddy covariance is on, but the WPL density')
            call LogSayList(' Warning(114)> correction is off, so the raw series are not converted')
            call LogSayList(' Warning(114)> to mixing ratios before the octants are built.')
            call LogSayList(' Warning(114)> The method sorts each air parcel by the SIGN of its')
            call LogSayList(' Warning(114)> water and carbon dioxide fluctuations. In a molar')
            call LogSayList(' Warning(114)> density from an open-path analyser, part of that')
            call LogSayList(' Warning(114)> fluctuation is the air expanding and contracting rather')
            call LogSayList(' Warning(114)> than the gas arriving, and that part is large enough to')
            call LogSayList(' Warning(114)> reverse the sign - so parcels land in the wrong octant')
            call LogSayList(' Warning(114)> and the ratio is drawn from the wrong points.')
            call LogSayList(' Warning(114)> Zahn et al. (2022) require the fluctuations themselves')
            call LogSayList(' Warning(114)> to carry the density correction, separately from the')
            call LogSayList(' Warning(114)> totals. Switch WPL on, or read the partition knowing')
            call LogSayList(' Warning(114)> this. A closed-path analyser already reporting mixing')
            call LogSayList(' Warning(114)> ratios is unaffected.')

        case(115)
            call LogSayList(' Warning(115)> The conditional eddy covariance significance test is on,')
            call LogSayList(' Warning(115)> but random uncertainty estimation is off, so there is no')
            call LogSayList(' Warning(115)> random error for it to test a flux against. The test is')
            call LogSayList(' Warning(115)> being skipped: no period is refused by it.')
            call LogSayList(' Warning(115)> Set the random uncertainty method to Finkelstein & Sims')
            call LogSayList(' Warning(115)> (1) to switch it on. That estimator uses each period''s')
            call LogSayList(' Warning(115)> own integral timescale, which is what makes the test a')
            call LogSayList(' Warning(115)> statement about that period rather than about an assumed')
            call LogSayList(' Warning(115)> eddy size.')
            call LogSayList(' Warning(115)> Everything else about the partition is unaffected.')
        ...
        case(110)
            call LogSayList(' Warning(110)> The spectral assessment succeeded for some gases and')
            call LogSayList(' Warning(110)> not for others. The assessment file was written and')
            call LogSayList(' Warning(110)> covers only the gases that were fitted; the rest are')
            call LogSayList(' Warning(110)> corrected with the analytical method instead, which')
            call LogSayList(' Warning(110)> reads instrument geometry rather than their measured')
        ...
        case(116)
            call LogSayList(' Warning(116)> The acquisition frequency changes within this averaging')
            call LogSayList(' Warning(116)> period. Its files cannot be joined into one time series,')
            call LogSayList(' Warning(116)> so the period is skipped. The next period starts with')
            call LogSayList(' Warning(116)> the file named above, at its own frequency.')
        case(117)
            call LogSayList(' Warning(117)> This period is at a higher acquisition frequency than the')
            call LogSayList(' Warning(117)> frequency grid of the binned (co)spectra was built for, a')
            call LogSayList(' Warning(117)> frequency the startup survey of the raw files did not see.')
            call LogSayList(' Warning(117)> Binned spectra, cospectra and ogives of such periods stop')
            call LogSayList(' Warning(117)> at the grid''s Nyquist frequency. Fluxes are not affected.')
        case(118)
            call LogSayList(' Warning(118)> The dynamic metadata file states an acquisition frequency')
            call LogSayList(' Warning(118)> or file duration that differs from the one embedded in')
            call LogSayList(' Warning(118)> the GHG files. The GHG files'' own values are used, as')
            call LogSayList(' Warning(118)> they describe the data actually recorded.')
        case(119)
            call LogSayList(' Warning(119)> The raw files are not all at one acquisition frequency')
            call LogSayList(' Warning(119)> for the gases listed above. Their spectral assessment is')
            call LogSayList(' Warning(119)> done separately at each of their rates: the noise floor,')
            call LogSayList(' Warning(119)> the transfer function and the Nyquist check each only up')
            call LogSayList(' Warning(119)> to that rate''s own Nyquist frequency. Each period is')
            call LogSayList(' Warning(119)> corrected with the result for its own rate; a rate with')
            call LogSayList(' Warning(119)> no usable result of its own takes the next faster one,')
            call LogSayList(' Warning(119)> as listed during the assessment. The assessment file')
            call LogSayList(' Warning(119)> carries one column set per rate for these gases.')
        case(112)
