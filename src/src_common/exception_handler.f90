...
subroutine ExceptionHandler(error_code)
    ...
    select case (error_code)
        ...
        case(99)
            write(*,*) '  Fatal error(99)> The project file (*.eddyflow) states no gas count.'
            write(*,*) '  Fatal error(99)> Gases, cell measurements and diagnostics are described'
            write(*,*) '  Fatal error(99)> by indexed records from format version 5.0.0 onward, and'
            write(*,*) '  Fatal error(99)> a file predating them would process no gas fluxes at all.'
            write(*,*) '  Fatal error(99)> This is not the same as a project that states gas_num=0:'
            write(*,*) '  Fatal error(99)> a site with an anemometer and no analyser is valid and is'
            write(*,*) '  Fatal error(99)> processed for sensible heat flux.'
        ...
        case(104)
            write(*,*) ' Warning(104)> This project measures gases but has no humidity: no gas'
            write(*,*) ' Warning(104)> record names H2O, and no biomet relative humidity column'
            write(*,*) ' Warning(104)> is selected. Two things follow, and neither is visible in'
            write(*,*) ' Warning(104)> the output.'
            write(*,*) ' Warning(104)> Air density and heat capacity are computed for DRY air, so'
            write(*,*) ' Warning(104)> density is overestimated and every density-based correction'
            write(*,*) ' Warning(104)> carries that bias.'
            write(*,*) ' Warning(104)> And the humidity correction to sensible heat flux cannot be'
            write(*,*) ' Warning(104)> applied, so the reported H is the uncorrected buoyancy flux,'
