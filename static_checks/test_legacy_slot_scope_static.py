...
`co2 = 5, h2o = 6, ch4 = 7, n2o = 8, gas4 = 8` are gone. They read as species
...
first. The same four numbers survive as `histCO2`, `histH2O`, `histCH4`,
`histN2O` and `histGas4`, so each remaining use says what it is.
...
What must not happen is a *new* `Set(:, histCO2)` or `E2Col(histH2O)` in a
...
SLOT_NAMES = re.compile(r"\b(histCO2|histH2O|histCH4|histN2O|histGas4)\b")
...
class TheSpeciesSpellingsAreGone(unittest.TestCase):
    ...
    def test_no_source_declares_or_uses_the_old_names(self):
        ...
        self.assertFalse(
            ...
            + "\n  ".join(offenders)
            + "\n\nUse histCO2/histH2O/histCH4/histN2O/histGas4 if a "
              "historical position is genuinely meant, or firstGas..lastGas "
              "and the gas record if a species is.")

    def test_the_historical_slots_are_declared(self):
        src = (ROOT / "src/src_common/m_typedef.f90").read_text(
            encoding="utf-8", errors="replace")
        for name in ("histCO2", "histH2O", "histCH4", "histN2O", "histGas4"):
            self.assertRegex(
                src, r"integer, parameter :: %s\s*=" % name,
                "%s is still needed by the fixed output format and the "
                "on-disk readers" % name)
